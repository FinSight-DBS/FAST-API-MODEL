...
class RunMonthlyUseCase:
    async def execute(self, request: RunMonthlyRequest) -> None:
        ...
        async with AsyncSessionLocal() as db:
            ...
            logger.info(f"[{job_id}] Processing {len(customer_ids)} customers for {target_month_str}")

            for chunk_start in range(0, len(customer_ids), CHUNK_SIZE):
                chunk = customer_ids[chunk_start: chunk_start + CHUNK_SIZE]
                await self._process_chunk(
                    job_id, chunk, month_start, month_end, target_month_str,
                    trx_repo, report_repo, customer_repo, request.dry_run,
                )
    ...
    async def _process_chunk(
        ...
        transactions = await trx_repo.find_debit_in_month(customer_ids, month_start, month_end)
        if not transactions:
        ...
        for customer_id in customer_ids:
            ...
            try:
                await self._generate_report(
                    job_id, customer_id, customer_debit, customer_all,
                    target_month_str, report_repo, customer_repo, dry_run,
                )
    ...
    async def _generate_report(
        ...
        features = compute_monthly_features(
            ...
            df_all=customer_all,
            gaji=0.0,
            prev_month_balance=float(customer_all["running_balance"].iloc[0]) if not customer_all.empty else 0.0,
        )
        persona_baru = predict_persona(features)

        total_pengeluaran = int(customer_debit["amount"].sum())
        wants_nom = int(customer_debit[customer_debit["main_category"] == "wants"]["amount"].sum())
        needs_nom = int(customer_debit[customer_debit["main_category"] == "needs"]["amount"].sum())
        wants_ratio = wants_nom / total_pengeluaran if total_pengeluaran > 0 else 0.0
        needs_ratio = needs_nom / total_pengeluaran if total_pengeluaran > 0 else 0.0

        saldo_akhir = float(customer_all["running_balance"].iloc[-1]) if not customer_all.empty else 0.0
        ...
        if not dry_run:
            ...
            try:
                ...
                logger.warning(f"[{job_id}] Failed to update persona for {customer_id}: {e}")
            logger.info(f"[{job_id}] Saved monthly report {saved_id} for {customer_id}, persona={persona_baru}")
        else:
